...
def get_student_timeline(student_id: str, month: int, year: int) -> list:
    ...
    # Fetch sessions that fall within the requested month
    ...
    # Build map of class_id -> {name, faculty_name}
    classes_map = {}
...
def get_student_session_history(student_id: str, page: int = 1, limit: int = 15) -> dict:
    ...
    Each entry shows: date, time, subject, status, marked_by, confidence.

    Returns:
        {'total': int, 'page': int, 'limit': int, 'sessions': [...]}
    """
    db = get_db()
    skip = (page - 1) * limit
    ...
    # Batch-fetch classes and teachers
